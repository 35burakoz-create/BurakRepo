...
import { buildCollectionMessageDraft, type MessageTone } from '../_shared/ai_utils.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    const quota = await checkAndConsumeMonthlyQuota({
      ...
      userId,
      featureName: 'collection_message',
    });
    if (!quota.allowed) {
    ...
    const response = buildCollectionMessageDraft(payload);

    await adminClient.from('ai_insights').insert({
      workspace_id: payload.workspace_id,
      insight_type: 'collection_message_draft',
      payload: {
        tone: payload.tone,
        contact_token: payload.contact,
    ...
    await logAiEvent({
      ...
      workspaceId: payload.workspace_id,
      featureName: 'collection_message',
      promptVersion: 'v2_collection_message',
      success: true,
