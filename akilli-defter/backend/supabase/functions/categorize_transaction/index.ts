...
import { heuristicCategory } from '../_shared/ai_utils.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    const quota = await checkAndConsumeMonthlyQuota({
      ...
      userId,
      featureName: 'categorize_transaction',
    });
    if (!quota.allowed) {
    ...
    const heuristic = heuristicCategory(`${payload.text} ${payload.merchant ?? ''}`);

    const { data: category } = await userClient
      .from('categories')
      .select('id,name')
      .eq('workspace_id', payload.workspace_id)
      .ilike('name', heuristic.category)
      .limit(1)
      .maybeSingle();
    ...
    await logAiEvent({
      ...
      workspaceId: payload.workspace_id,
      featureName: 'categorize_transaction',
      promptVersion: 'v2_categorize_transaction',
      success: true,
