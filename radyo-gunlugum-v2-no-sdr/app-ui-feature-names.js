...
function refresh(){renameMenu();renameSmartSurface();renameAudioSurface();renameAiSurface()}
R.events?.on?.('menu:opened',x=>renameMenu(x?.sheet));
R.events?.on?.('route:changed',x=>{if(['smart','audio','ai'].includes(x?.to))setTimeout(refresh,0)});
for(const event of ['render:all','data:loaded','listening:data','ai:data'])R.events?.on?.(event,()=>setTimeout(refresh,0));
R.uiFeatureNames={names:NAMES,renameMenu,renameSmartSurface,renameAudioSurface,renameAiSurface,refresh};
