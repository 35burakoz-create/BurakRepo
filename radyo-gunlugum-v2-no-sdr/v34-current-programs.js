...
function remaining(n,r){let end=Number(r?.[1]);if(!Number.isFinite(end))return null;let diff=end-n.mins;if(Number(r[0])>end&&n.mins>=Number(r[0]))diff=(1440-n.mins)+end;if(end===1440)diff=1440-n.mins;if(diff<0)diff+=1440;return diff}
...
function hhmm(v){v=Number(v);if(v===1440)return'24:00';v=((v%1440)+1440)%1440;return`${String(Math.floor(v/60)).padStart(2,'0')}:${String(v%60).padStart(2,'0')}`}
function rangeText(r){return r?`${hhmm(r[0])}–${hhmm(r[1])}`:''}
function score(e){let s=Number(e.probability_score||50);try{const tol=e.mode==='FM'?.25:e.mode==='MW'?12:30,h=(R.logs||[]).filter(x=>(e.mode==='SW'?String(x.band).startsWith('SW'):x.band===e.band)&&Math.abs(Number(x.frequency)-Number(e.frequency))<=tol&&x.signal_strength);if(h.length){const av=h.reduce((z,x)=>z+Number(x.signal_strength),0)/h.length;s+=Math.round((av-2.5)*5)}}catch{}return Math.max(1,Math.min(99,Math.round(s)))}
...
function segmentProgram(e,n){const parts=String(e.time_text||'').split(';').map(x=>x.trim()).filter(Boolean),ranges=Array.isArray(e.time_ranges)?e.time_ranges:[];if(parts.length<2||parts.length!==ranges.length)return'';const i=ranges.findIndex(r=>inRange(n.mins,r));if(i<0)return'';const m=parts[i].match(/\d{1,2}[.:]\d{2}\s*[–-]\s*\d{1,2}[.:]\d{2}\s+(.+)$/u);if(!m)return'';const x=m[1].trim();if(!x||/^yaklaşık\b/i.test(x))return'';return x}
function card(e){const n=trNow(),s=Math.max(1,Math.min(99,Math.round(e._score||50))),band=e.mode==='SW'?(e.band||'SW'):e.mode,rem=remainText(remaining(n,e._range)),prog=segmentProgram(e,n);return`<article class=v33-card><span class=v33-band>${esc(band)}</span><div class=v33-freq>${frequency(e)}</div><div class=v33-station>${esc(e.station||'Aday yayın')}</div>${prog?`<div class=v34-program>▶ Şu anki program: ${esc(prog)}</div>`:''}<div class=v34-meta2>${contentLines(e)}${e.country?`<div><b>Ülke:</b> ${esc(e.country)}</div>`:''}</div><div class=v34-time><span class=on>● Şimdi yayında</span><span>🕒 ${rangeText(e._range)}</span>${rem?`<span>${esc(rem)}</span>`:''}</div><div class=v33-score><span>${s}/99</span><span class=v33-bar><i style="width:${s}%"></i></span></div><button class="btn ghost" data-v33try="${e.id}">Bu yayını dene</button></article>`}
