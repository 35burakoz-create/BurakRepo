...
// Load the Focus UX, persistent navigation, Radio Memory and Propagation Assistant after integrity layers are ready.
function loadOnce(src){if(document.querySelector(`script[src="${src}"],script[src="./${src}"]`))return Promise.resolve();return new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.onload=resolve;s.onerror=()=>reject(new Error(src+' yüklenemedi'));document.head.appendChild(s)})}
(async()=>{for(let i=0;i<100;i++){if(typeof R.radioNowCandidates==='function'&&Object.prototype.hasOwnProperty.call(R,'v36Settings'))break;await new Promise(r=>setTimeout(r,50))}try{await loadOnce('v38-listening-mode.js');await loadOnce('v38-ux-shell.js');await loadOnce('v39-navigation-state.js');await loadOnce('v40-radio-memory.js');await loadOnce('v41-propagation-assistant.js')}catch(e){console.error('V3.5 Focus UX yüklenemedi',e)}})();
})();