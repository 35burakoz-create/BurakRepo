...
function toast(message,{type='info',duration=2600}={}){close();node=document.createElement('div');const isError=type==='error';node.className=`app-toast ${isError?'error':type==='success'?'success':''}`.trim();node.setAttribute('role',isError?'alert':'status');node.setAttribute('aria-live',isError?'assertive':'polite');node.setAttribute('aria-atomic','true');node.textContent=String(message??'');document.body.appendChild(node);timer=setTimeout(close,Math.max(isError?1800:900,Number(duration)||2600));return node}
