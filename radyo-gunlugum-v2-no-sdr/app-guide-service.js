...
function wallTimeToInstant(date,time){
  ...
  const target=Date.UTC(y,mo-1,d,h||0,mi||0),fmt=new Intl.DateTimeFormat('en-CA',{timeZone:TZ,year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hourCycle:'h23'});
  let ts=target;
  for(let i=0;i<3;i++){const p=Object.fromEntries(fmt.formatToParts(new Date(ts)).filter(x=>x.type!=='literal').map(x=>[x.type,x.value])),seen=Date.UTC(+p.year,+p.month-1,+p.day,+p.hour,+p.minute),delta=target-seen;ts+=delta;if(!delta)break}
  return new Date(ts)
}
...
function activeAt(e,date,time){const s=scheduleState(e,date,time);if(s)return s.active;if(!weekdayOK(e,date,time)||!validAt(e,date,time))return false;if(Array.isArray(e?.time_ranges)&&e.time_ranges.length)return e.time_ranges.some(r=>inRange(minuteOf(time),r));return e?.mode==='FM'}
