...
function updateDock(route){document.querySelectorAll('#v38Dock [data-route]').forEach(b=>b.classList.toggle('active',b.dataset.route===route))}
...
const router={
 ...
 go(name,meta={}){
   ...
   try{
     ...
     const base=R.coreSwitch;if(typeof base!=='function')throw new Error('Pristine core switch is unavailable');
     base(target);current=visibleRoute()||target;
