(()=>{
const R=window.R;if(!R||R.__routerCore386)return;R.__routerCore386=true;const events=R.events,$=s=>document.querySelector(s),$$=s=>[...document.querySelectorAll(s)];
...
function hashRoute(){let h='';try{h=decodeURIComponent(location.hash||'')}catch(error){R.reportError?.(error,'router-hash',{silent:true});return null}h=h.replace(/^#/,'').replace(/^tab=/,'');return KNOWN.has(h)?h:null}
...
function updateDock(route){$$('#v38Dock [data-route],#appDesktopNav [data-route]').forEach(b=>{const active=b.dataset.route===route;b.classList.toggle('active',active);if(active)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current')})}
...
const router={
 ...
 sync(){const candidate=hashRoute()||visibleRoute()||R.uiState?.tab||current,next=router.has(candidate)?candidate:'home';if(next!==current){const prev=current;current=next;updateDock(current);events?.emit?.('route:changed',{from:prev,to:current,source:'sync',historyMode:'none',restorePosition:false})}return current},
 switchView
};
...
$$('.main-tab').forEach(b=>b.onclick=null);document.addEventListener('click',e=>{const target=e.target instanceof Element?e.target:null;if(!target)return;const b=target.closest('.main-tab[data-tab]');if(!b)return;e.preventDefault();router.go(b.dataset.tab,{source:'main-tab',historyMode:'push'})},true);
