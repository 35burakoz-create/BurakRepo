...
async function load({force=false,sync=true}={}){if(!R.me){state.rows=[];state.loaded=false;state.userId=null;R.achievements=[];return state}if(!force&&state.loaded&&state.userId===R.me.id){if(sync)await syncEarned();return state}if(state.loading)return state.loading;state.loading=(async()=>{const q=await R.S.from('radio_achievements').select('*').eq('user_id',R.me.id).order('unlocked_at',{ascending:false});if(q.error)throw q.error;state.rows=q.data||[];state.loaded=true;state.userId=R.me.id;R.achievements=state.rows;R.events?.emit?.('achievements:data',{count:state.rows.length,total:DEF.length});if(sync)await syncEarned();return state})().finally(()=>state.loading=null);return state.loading}
async function syncEarned(){if(!R.me||state.syncing)return state.syncing;state.syncing=(async()=>{const s=stats(),old=unlocked(),earned=DEF.filter(d=>{try{return d.test(s)&&!old.has(d.key)}catch{return false}});if(!earned.length)return[];const q=await R.S.from('radio_achievements').insert(earned.map(d=>({user_id:R.me.id,achievement_key:d.key,metadata:{title:d.title}}))).select();if(q.error){R.reportError?.(q.error,'achievement-sync',{silent:true});return[]}const fresh=await R.S.from('radio_achievements').select('*').eq('user_id',R.me.id).order('unlocked_at',{ascending:false});if(!fresh.error){state.rows=fresh.data||[];R.achievements=state.rows}R.events?.emit?.('achievements:unlocked',{earned:earned.map(x=>x.key),count:state.rows.length});if(R.toast)R.toast(earned.length===1?`🏆 Yeni rozet: ${earned[0].title}`:`🏆 ${earned.length} yeni rozet açıldı`);return earned})().finally(()=>state.syncing=null);return state.syncing}
