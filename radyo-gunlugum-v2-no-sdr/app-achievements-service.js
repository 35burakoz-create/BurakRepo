...
const DEF=[
 ...
 ['night_owl','🦉','Gece MW','00.00–04.59 arasında bir MW yayını kaydet.',()=>R.logs.some(x=>{const h=hourOf(x);return x.band==='MW'&&h!=null&&h<5})],
...
function applyRows(rows,userId){if(R.me?.id!==userId)return false;state.rows=rows||[];state.loaded=true;state.userId=userId;R.achievements=state.rows;R.events?.emit?.('achievements:data',{count:state.rows.length,total:DEF.length});return true}
...
R.achievementDefs=DEF;R.achievements=state.rows;R.achievementsService={state,definitions:DEF,stats,progress,load,sync:syncEarned,validDate,hourOf,maxStreak};
