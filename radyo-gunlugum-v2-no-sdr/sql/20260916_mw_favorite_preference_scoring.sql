-- MW “Şu An” sıralamasına kullanıcının açık favori tercihini sınırlı ve açıklanabilir biçimde ekler.
-- Favori katkısı +4 puanla sınırlıdır; mesafe/yayılım/aktiflik gibi fiziksel ve zamansal sinyalleri bastırmaz.
-- Eşleşme öncelikle canonical_station_id + frekans üzerinden, eski/çözümlenmemiş kayıtlarda aynı frekanstaki ad üzerinden yapılır.
...
begin
  ...
  with active_ids as materialized (
    ...
    select b.*,
      ...
      greatest(0,least(100,b.distance_score+b.power_bonus+b.antenna_bonus+b.target_bonus+b.p_adjustment+b.favorite_bonus+case when (b.eibi_active or b.official_active) then 8 else -18 end))::int as final_score
    from base b
  ), ranked as (
