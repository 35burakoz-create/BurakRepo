...
function hasDraft(userId=R.me?.id){const key=draftStorageKey(userId);if(!key)return false;try{return!!localStorage.getItem(key)}catch{return false}}
function clearDraftFor(userId=R.me?.id,{resetVisible=true}={}){const key=draftStorageKey(userId);if(!key)return false;try{localStorage.removeItem(key)}catch{return false}if(userId===R.me?.id){draftDirty=false;$('#v39DraftBanner')?.remove();if(resetVisible)resetVisibleForm()}return true}
