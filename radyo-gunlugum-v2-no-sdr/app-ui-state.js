(()=>{
const R=window.R;if(!R||R.__uiState386)return;R.__uiState386=true;
...
function resetScroll(){setTimeout(()=>window.scrollTo({top:0,left:0,behavior:'auto'}),0)}
...
R.events?.on?.('route:changed',ctx=>{if(!ctx?.to||!VALID.has(ctx.to))return;state.tab=ctx.to;save();if(!browserRouting)writeHash(ctx.to,ctx.historyMode||'replace');if(ctx.to==='now')applyNowMode();if(ctx.restorePosition)restoreScroll(ctx.to);else if(ctx.from&&ctx.from!==ctx.to)resetScroll()});
...
function hasDraft(userId=R.me?.id){const key=draftStorageKey(userId);if(!key)return false;try{return localStorage.getItem(key)!==null}catch{return false}}
function clearDraftFor(userId=R.me?.id){const key=draftStorageKey(userId);if(!key)return false;try{localStorage.removeItem(key);if(userId===R.me?.id)localStorage.removeItem(DRAFT_KEY)}catch{return false}if(userId===R.me?.id){draftDirty=false;$('#v39DraftBanner')?.remove()}return true}
function clearDraft(){if(suppressDraftClear)return false;return clearDraftFor()}
...
function resetVisibleForm(){suppressDraftClear=true;try{R.reset?.()}finally{suppressDraftClear=false;draftDirty=false}$('#v39DraftBanner')?.remove()}
...
R.uiStatePersistence={saveDraft,restoreDraft,clearDraft,clearDraftFor,hasDraft,draftStorageKey,stateStorageKey,applyStoredState,setFilter,save,resetVisibleForm,restoreScroll,resetScroll,readHash};
