...
for(const asset of ['./app-branding-experience.js','./app-branding-experience.css','./assets/branding/r9012-splash.svg','./assets/branding/r9012-share-cover.svg'])assert(sw.includes(`'${asset}'`),`service worker must cache ${asset}`);
...
assert(js.includes("installShareMeta(){const cover=new URL(fallback('share_cover')"),'social metadata must use the durable packaged cover, not an expiring signed URL');
