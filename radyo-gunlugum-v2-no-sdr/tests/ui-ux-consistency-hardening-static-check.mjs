...
const swGeneration=sw.match(/PWA_CACHE_GENERATION='([^']+)'/)?.[1];
check('PWA config has a unique build generation',config.includes("cacheVersion:'v385-core-boundary-radio-intelligence-20260915-52'")&&!!configBuild&&configBuild!=='legacy'&&config.includes('dataset.appBuild=config.buildId'));
check('service worker cache name consumes the central build generation',sw.includes("const BUILD_ID=String(CFG.buildId||'legacy')")&&sw.includes('const CACHE=`radyo-${CFG.cacheVersion}-${BUILD_ID}`')&&!!swGeneration&&swGeneration===configBuild);
