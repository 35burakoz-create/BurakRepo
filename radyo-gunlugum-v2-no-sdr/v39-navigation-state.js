...
const DYNAMIC=new Set(['home','now','memory','propagation']);
...
function switchCore(tab,{historyMode='replace',restorePosition=false}={}){
 tab=validTarget(tab);if(!exists(tab)&&!DYNAMIC.has(tab))tab='home';
...
function enforceRoute(){if(!R.me||$('#appView')?.classList.contains('hidden'))return;const desired=validTarget(state.tab);if(!exists(desired)&&!DYNAMIC.has(desired)){state.tab='home';save();return switchCore('home',{historyMode:'replace'})}if(DYNAMIC.has(desired)&&!['home','now'].includes(desired)&&!exists(desired))return;const v=visibleTab();if(v!==desired)switchCore(desired,{historyMode:'replace',restorePosition:false});else{current=desired;writeHash(desired,'replace');if(desired==='now')applyNowMode()}}
...
function bootRestore(){const desired=readHash()||state.tab||'home';state.tab=validTarget(desired);save();restoreFilters();restoreDraft();if(R.me&&!$('#appView')?.classList.contains('hidden')){if(DYNAMIC.has(state.tab)&&!['home','now'].includes(state.tab)&&!exists(state.tab))writeHash(state.tab,'replace');else switchCore(state.tab,{historyMode:'replace',restorePosition:true})}else writeHash(state.tab,'replace')}
